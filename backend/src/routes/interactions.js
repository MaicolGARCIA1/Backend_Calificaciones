...
router.post('/save-rating', (req, res) => {
  const { id_estudiante, tipo_interaccion, comentario, calificacion } = req.body;
  ...
  const query = 'INSERT INTO Interacciones (id_estudiante, tipo_interaccion, comentario, calificacion) VALUES (?, ?, ?, ?)';
  db.query(query, [id_estudiante, tipo_interaccion, comentario, calificacion], (err, result) => {
    if (err) {
